...
function stats(data) {
  ...
  profileHeader.innerHTML = `<h2>${stockName}&nbsp&nbsp<span style="color: green;">${data[
    stockName
  ].profit.toFixed(2)}%&nbsp&nbsp</span>$${data[stockName].bookValue.toFixed(
    2
  )}</h2>`;
  ...
  for (const key in data) {
    ...
    if (key != "_id") {
      anchor.addEventListener("click", () => {
        ...
        newChart();
        profile.innerText = profileData[stockName].summary;
        profileHeader.innerHTML = `<h2>${stockName}&nbsp&nbsp<span style="color: ${color};">${data[
          stockName
        ].profit.toFixed(2)}%&nbsp&nbsp</span>$${data[
          stockName
        ].bookValue.toFixed(2)}</h2>`;
      });
